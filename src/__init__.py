...
path = pathlib.Path(r"e:\Document\PROJECT\notebooks\06_gru.ipynb")
...
updates = {
    ...
    17: src(
        ...
        print("Bộ đối chiếu optimistic:", OPTIMISTIC_FEATURE_SET)
    ...
    19: src(
        ...
        # Tạo cửa sổ trượt cho bài toán multi-step forecasting.
    ...
    22: src(
        ...
        # Scale feature/target và tạo sequence cho huấn luyện và validation nội bộ.
        ...
        def build_inference_input(history_raw_df, feature_cols, x_scaler, lookback=72):
            ...
            if len(feature_frame) < lookback:
                raise ValueError("Không đủ lịch sử sau khi tạo feature để dựng input inference.")
        ...
        def run_assimilated_rollout(
            ...
            if len(future_raw_df) < rollout_horizon:
                raise ValueError("Không đủ dữ liệu tương lai để rollout đủ số bước yêu cầu.")
            ...
            for chunk_start in range(0, rollout_horizon, chunk_horizon):
                ...
                if len(chunk_future) < chunk_horizon:
                    raise ValueError("Chunk cuối không đủ số bước dự báo.")
            ...
            if len(rollout_df) != rollout_horizon:
                raise ValueError("Rollout không trả đúng số bước yêu cầu.")
    ...
    23: src(
        ...
        if len(test_rollout_raw) < ROLLOUT_HORIZON:
            raise ValueError("Không đủ dữ liệu test để rollout đủ 72h.")
    ...
    25: src(
        ...
        # Đóng gói kết quả đã có sẵn từ cell test chính để tránh train lại một mô hình.
        ...
        # Huấn luyện và đánh giá một biến thể model trên production pipeline.
        ...
        for chunk_boundary in range(CHUNK_HORIZON, ROLLOUT_HORIZON, CHUNK_HORIZON):
            ...
            plt.axvline(boundary_ts, color="#999999", linestyle="--", linewidth=1.0, alpha=0.7)
        plt.title("Production-style 72h rollout tren test")
        ...
        for chunk_boundary in range(CHUNK_HORIZON, ROLLOUT_HORIZON, CHUNK_HORIZON):
            ...
            plt.axvline(boundary_ts, color="#999999", linestyle="--", linewidth=1.0, alpha=0.7)
        plt.title("Zoom toan bo rollout 72h tren test")
        plt.xlabel("Thời gian")
        plt.ylabel("PM2.5")
        plt.legend()
        plt.grid(alpha=0.3)
...
for idx, source in updates.items():
